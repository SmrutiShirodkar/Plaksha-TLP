...
class myserver_bad_state_change(Exception):
    def __init__(self, server, newstate, message="Active VMs are present"):
        ...
        self.message=message
    def __str__(self):
        return f'{self.server.sr} oldstate: {self.state} newstate: {self.newstate} vmsactive: {self.nvms} -> {self.message}'
...
# and the members listed above . 
    ...
    def set_fail(self):
        if self.state == 'fail':
            raise Exception("Sever already in failed state")
        if self.state == 'idle':
            self.stats['sidle']-=1
    ...
    def set_active(self):
        if self.state == 'active':
            raise Exception("Sever already in active state")        
        if self.nvms>0:
            self.state='active'
            if self.state=='idle':
                self.stats['sidle']-=1
    ...
    # deprovision the vm on this server. Recover the mem and cpu given.
    ...
    def deprovision_vm(self, vm):
        self.vl.delete(vm)
        self.nvms-=1    
        if self.state=='active' and self.nvms==0:
            self.state='idle'
            self.stats['sidle']+=1
            self.stats['sactive']-=1
        self.amem += vm.amem
    ...
    def commit_vm(self, vm):
        if vm.state=='provision':
            ...
            raise Exception("VM not provisioned to the server")
        # raise NotImplementedError
    ...
    def delete(self):
        if self.nvms!=0:
            raise Exception("Server ",self.sc," cannot be deleted if VMs are active/provisioned.")
        if self.state == 'idle':
            self.stats['sidle']-=1
        if self.state == 'active':
            self.stats['sactive']-=1
    ...
    def unfail(self):
        if self.state!='fail':
            raise Exception("Server ", self.sc,"not in failed state")
        self.state='idle'
        self.stats['sfail']-=1
        self.stats['sidle']+=1
        # raise NotImplementedError
